...
export class Space extends EventEmitter {
  ...
  public async initialize(config: SpaceConfig) {
    ...
    const region = await getRegion(this.scraper.getFetch());
    ...
    this.authToken = await authorizeToken(cookie, this.scraper.getFetch());

    // 4) Gather TURN servers
    this.logger.debug('[Space] Getting turn servers...');
    const turnServers = await getTurnServers(cookie, this.scraper.getFetch());
