...
export class SpaceParticipant extends EventEmitter {
  ...
  public async joinAsListener(): Promise<void> {
    ...
    this.authToken = await authorizeToken(this.cookie, this.scraper.getFetch());
    ...
    const source = status.source as { location: string } | undefined;
    this.hlsUrl = source?.location;
    this.chatJwtToken = status.chatToken as string | undefined;
    this.lifecycleToken = status.lifecycleToken as string | undefined;
    ...
    const chatInfo = await accessChat(
      this.chatJwtToken,
      this.cookie!,
      this.scraper.getFetch(),
    );
    ...
    this.watchSession = await startWatching(
      this.lifecycleToken!,
      this.cookie!,
      this.scraper.getFetch(),
    );
  ...
  public async becomeSpeaker(): Promise<void> {
    ...
    const turnServers: TurnServersInfo = await getTurnServers(
      this.cookie!,
      this.scraper.getFetch(),
    );
