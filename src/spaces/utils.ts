...
export async function publishBroadcast(
  ...
  await authFetch('https://proxsee.pscp.tv/api/v2/publishBroadcast', {
    method: 'POST',
    headers,
    body: JSON.stringify({
      accept_guests: true,
...
export async function getTurnServers(
  ...
  const resp = await authFetch('https://proxsee.pscp.tv/api/v2/turnServers', {
    method: 'POST',
    headers,
 ...
 */
export async function getRegion(authFetch: typeof fetch): Promise<string> {
  const resp = await authFetch('https://signer.pscp.tv/region', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Referer: 'https://x.com',
 ...
 */
export async function negotiateGuestStream(
  params: {
    broadcastId: string;
    sessionUUID: string;
    authToken: string;
 ...
 * If called by a speaker, pass your own sessionUUID.
 */
export async function unmuteSpeaker(
  params: {
    broadcastId: string;
    sessionUUID?: string;
    chatToken: string;
